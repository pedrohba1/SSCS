...
// creates a new composite core, containing the basic setup: a recorder, a recognizer, an indexer and a storer.
// This core is composite, so it allows multiple recognition algorithms running at the same time.
func NewComposite(args []string) *Core {
	...
	v := recognizer.NewCompositeRecognizer(recognizer.EventChannels{
		FrameIn: frameChan,
		RecogOut: recogChan,
	})
