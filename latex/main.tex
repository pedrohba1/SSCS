...
Em Seguida, serão exploradas as tecnologias comumente adotadas na indústria.
Essa investigação abrange desde protocolos de transporte e arquiteturas
predominantes de hardware e software até algoritmos sofisticados de compressão
e reconhecimento. O intuito é discernir os padrões de uso e as preferências do
setor.
...
corrente constante de pacotes por UDP ou TCP. O Real Time Streaming Protocol
(RTSP) , baseado no Real-time Transport Protocol (RTP), é o mais comum para a
transmissão de vídeo nas câmeras por IP: vasta maioria delas o integram como
padrão. Já no método do streaming por
...
Delivery Network (CDN) para aumentar a escalabilidade. É comum em streaming adaptativo utilizar a técnica de 
\textit{\foreignlanguage{english}{stream switching}}. Esse é um método híbrido que usa HTTP
como o protocolo de entrega no lugar de definir um novo protocolo. Os dados de multimídia são
segmentados em pequenas partes de mesmo tamanho, codificados no formato
desejado e armazenadas em um servidor. Os clientes então requisitam os segmentos sequencialmente por
download progressivo e eles são tocados em ordem já que são contíguos. O resultado é uma experiência de usuário
