...
\begin{resumo} %TCC:
	...
	surge o chamado \textit{\foreignlanguage{english}{Self-Sovereign Camera
			System}} (SSCS). O desenvolvimento desse sistema resultou em componentes de
	software para a vigilância em vídeo tais como reconhecimento, armazenamento e
	indexação, todas disponíveis em código aberto.
...
\begin{resumo}[Abstract]
	...
	Camera System (SSCS) emerges. The development of this system resulted in
	software components for video surveillance such as recognition, storage, and
	indexing, all available as open source.
...
\begin{table}[ht]
	...
	\caption{Comparativo entre instâncias de máquinas virtuais em
		...
		\cite{awsCalc,gcpCalc,azureCalc}.}
	\label{tab:estima}
	\begin{tabular}{|l|l|l|}
		\hline
...
recebidos por meio dos canais. Do ponto de vista da arquitetura, o banco de
dados é uma não-entidade. É um detalhe que não se eleva ao nível de um elemento
...
%%Um detalhe pertinente é o fato do indexador ser agnóstico quanto ao banco de dados usado.% FICOU ESTRANHA A FRASE ABAIXO
...
\section{Implementação do detector usando cascata de Haar}
...
\begin{apendicesenv}
	...
	Neste apêndice, faz-se a cobertura completa por todas as
	...
	desenvolvimento\footnote{Para mais informações sobre o ambiente de
		desenvolvimento, consulte a \autoref{sec:amb}}, portanto, os modelos de
	configuração podem ser usados como suas configurações padrão. À medida que as
	partes do sistema são executadas em condições diferentes, ou seja, servidores
	remotos, métodos de autenticação alternativos, estrutura de armazenamento
	alternativa e assim por diante, o usuário terá que ajustá-los em tempo real.

	\section{Inicialização de todos os componentes básicos}
	...
	para executá-lo em desenvolvimento também está disponível no \texttt{makefile}
	e pode ser executado com \texttt{make key-cloak}.
...
\end{minted}
	...
	executado tanto através do binário quanto executando \texttt{go run
		./cmd/daemon}. Observe que será utilizado o \texttt{sscs.yml} que está presente
	no diretório raiz. Portanto, certifique-se de executar no caminho correto. Deve
	começar a exibir \textit{logs}, conforme exemplo:
...
\end{minted}
	...
	Também é possível instalar como um \texttt{Daemon} para que continue
	funcionando enquanto o Sistema Operacional estiver em execução:
...
\end{minted}
	...
	\begin{figure}[ht]
		...
		\begin{minipage}{0.48\textwidth}
			...
			\captionof{figure}{Tina, a gata, sendo reconhecida em
				um \texttt{Core}}
			\label{fig:tina}
		\end{minipage}\hfill
		\begin{minipage}{0.48\textwidth}
	...
	\begin{enumerate}
		\item Busca por todos os eventos de reconhecimento, permitindo
		      filtro por intervalo de datas usando datas no formato RFC3339. A API responde
		      com o contexto de reconhecimento, a data de criação do evento e um hyperlink
		      para a imagem do que foi reconhecido, com marcações.
...
\end{minted}
		...
		\item Busca por todas as gravações, com filtro de intervalo de
		      datas semelhante à funcionalidade anterior. A API fornece o horário de início e
		      fim de cada gravação, bem como um hyperlink para sua visualização.
