...
de rede, concatená-los e codificá-los em um arquivo único, com subsequente
armazenamento no disco ou nuvem em segmentos de arquivos. Estes arquivos são
registrados com uma timestamp, refletindo o instante de criação do arquivo de
gravação.
...
que o \texttt{Recorder} desempenhe suas operações. A gravação
também não é exclusiva a um protocolo específico: qualquer sistema que tenha a
capacidade de transmitir mídia utilizando os protocolos RTSP, WebRTC ou SRT
pode ser integrado também, assumindo que o componente de gravador dê suporte.
...
É possível vincular o gravador a um indexador,
...
transmissões de vídeo anteriores. É importante salientar também que a
implementação do indexador não se torna mandatória, a menos que corresponda a
uma necessidade específica do usuário. O indexador se integra com o gravador da seguinte maneira:
